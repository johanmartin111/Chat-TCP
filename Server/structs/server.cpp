...
void Server::on_readyRead(Client *C, Peticion p){
    ...
    case NUEVO_CHAT:{
        ...
        Chat *ch = new Chat(chats.size(), chat_clients);
        chats << ch;
        ch->startChat();

        chats.at(0)->addMsj( Mensaje(Usuario("Servidor"),"sala de chat creado...").toString() );
        Usuario u = C->getUsuario();
        Mensaje m = Mensaje(u, chats.at(0)->getMensajes().join("<br>"));
        broadcast(Peticion(MENSAJE, m));
        break;
    }
    case MENSAJE:{
        chats.at(0)->addMsj( p.getMsj().toString() );
